...
/* Product Functions start */ 
  public function edit_product_image(Request $request)
  {
    ...
    if($request->ids!=''){     
      $updateData = DB::table('home_products_gallery')->where('id', $request->ids)->update(['image' => $image]);
      session()->flash('success', 'Image Update successfully..!');
      return back();
    }else{
      $insertData = DB::table('home_products_gallery')->insertgetId(['image' => $image, 'home_product_id' => $request->product_id]);
  ...
  {
    $delete = DB::table('home_products_gallery')->where('id', $id)->delete();
  ...
  {
    $data = DB::table('home_products_gallery')->where('id', $id)->first();
  ...
  public function add_product(Request $request)
  {
    ...
    if($request->edit_id!=''){     
      $updateData = DB::table('home_products')->where('id', $request->edit_id)->update($data);
      Session::flash('success','Updated successfully..!');
      return redirect('product');
    }
    else{
      $insertData = DB::table('home_products')->insertgetId($data);
  ...
  {    
    if(Session::get('userRole') == 1){
      $productdata = DB::table('home_products')->orderBy('id', 'desc')->get();
    }else{
      $productdata = DB::table('home_products')->orderBy('id', 'desc')->where('creator_id', Session::get('user_id'))->get();
    }
  ...
  {
    $product_view_data = DB::table('home_products')->where('id', $id)->first();

    /*print_r($product_view_data); die;*/

    $data['content'] = 'admin.product.edit_products';
    return view('layouts.content', compact('data'))->with(['product_view_data' => $product_view_data ]);
  }
  ...
  {
    $delete = DB::table('home_products')->where('id', $id)->delete();
    session()->flash('error', 'Deleted Successfully..!');
    return redirect()->back();
  }
  ...
  {
    $data = DB::table('Service_Category')->where('Service_Id', $serviceid)->get();
    return response()->json($data);
  }

  public function get_service_activity($service_cat_id)
  {
    $data = DB::table('Service_Category_ActivityType')->where('Service_Category_Id', $service_cat_id)->get();
   /* print_r($data); die;*/
    return response()->json($data);
  }
