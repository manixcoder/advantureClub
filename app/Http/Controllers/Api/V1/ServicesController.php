...
class ServicesController extends MyController
{
    ...
    public function get_all(Request $request)
    {
        ...
        $service_category = DB::table('service_categories')->where('status', 1)->where('deleted_at', NULL)->get();
        ...
        foreach ($service_category as $category) {
            ...
            if (!$services->isEmpty()) {
                $service_array = array('category' => $category->category, 'services' => $services);
            }
        ...
        if (!$services->isEmpty()) {
            //return $this->sendResponse(config('constants.DATA_FOUND'), $service_array, 200);
        }
        //return $this->sendError('No record found.', [], 404);
    ...
    public function myServicereview(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $liked = DB::table('users')->leftJoin('service_reviews', 'users.id', '=', 'service_reviews.user_id')
                ->leftJoin('service_likes', 'users.id', '=', 'service_likes.user_id')->get();
    ...
    public function likeService(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $liked = DB::table('service_likes')->where(['user_id' => $request->user_id, 'service_id' => $request->service_id])->first();
            //dd($liked);
            if (!empty($liked)) {
                if ($request->like == 0) {
        ...
        if ($validator->fails()) {
            ...
            $booking = DB::table('bookings as bkng')
                ->select([
                    ...
                    'rgn.region',
                    'srvc.adventure_name',
                    'usr.name as provider_name', 'usr.height as height', 'usr.weight as weight', 'usr.health_conditions as health_conditions',
