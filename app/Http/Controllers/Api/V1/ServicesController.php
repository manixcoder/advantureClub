...
class ServicesController extends MyController
{
    ...
    public function likeService(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $liked = DB::table('service_likes')
                ->where(['user_id' => $request->user_id, 'service_id' => $request->service_id])
                ->first();
            //dd($liked);
            if (!empty($liked)) {
                if ($request->like == 0) {
    ...
    public function checkPromoCode(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $promocode = DB::table('promocode')
                ->select([
                    '*',
                    DB::raw("IF(discount_type = 'A','Amount','Percentage') AS discount_type")
                ])
                ->where([
