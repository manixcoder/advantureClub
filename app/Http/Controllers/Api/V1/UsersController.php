...
class UsersController extends MyController
{
    ...
    {
        $otp_model = new Otp();
        $otp = $otp_model->select(['user_id', 'id', 'otp', 'mobile'])->where(array('user_id' => $user_id, 'status' => 0, 'mobile' => $mobile))->first();
        if (!empty($otp)) {
    ...
    public function getOtp(Request $request)
    {
        ...
        $result = User::Select(array('id', 'users_role', 'name', 'mobile_code', 'mobile', 'mobile_verified_at', 'email', 'email_verified_at', 'profile_image', 'status'))
        ...
        if (!empty($result)) {
            ...
            if ($forgot_password) {
                $otp_model = new Otp();
                $otp = $otp_model->select(['user_id', 'id', 'otp'])->where(array('user_id' => $user_id, $field => $user_mobile, 'status' => 0))->first();
                if (!empty($otp)) {
                    $update = Otp::select(['user_id', 'id', 'otp'])->find($otp->id);
                    $otp_model = $update;
                }
        ...
        $otp_model = new Otp();
        $otp = $otp_model->select(['user_id', 'id', 'otp'])->where(array('user_id' => $user_id, $field => $request->mobile, 'status' => 0))->first();
        if (!empty($otp)) {
            $update = Otp::select(['user_id', 'id', 'otp'])->find($otp->id);
            $otp_model = $update;
        }
        ...
        $otp_model->save();
        if ($field == 'mobile') {
            $sentSms = $this->sendSms_mobile($request->mobile_code, $request->mobile, $otp_model->otp);
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if ($request->forgot_password) {
            $where = array('type' => 2, 'user_id' => $request->user_id, 'status' => 0, 'otp' => $request->otp);
        } else {
            $where = array('type' => 1, 'user_id' => $request->user_id, 'status' => 0, 'otp' => $request->otp);
        }

        $otp = Otp::select(['user_id', 'id', 'otp', 'otp_on', 'updated_at'])->where($where)->first();
        if (!empty($otp)) {
            if ($otp->otp_on == 2) {
                $otp_on = 'email';
            ...
            if ($request->forgot_password) {
                ...
                $update->delete();
                DB::table('otp')->whereRaw('status=1 && DATE_FORMAT(created_at, "%Y-%m-%d") < CURDATE()')->delete();
            }
            $user = User::select(['id', 'id as user_id', 'name', 'email', 'profile_image', 'mobile', 'mobile_verified_at', 'status'])->find($request->user_id);
            if (!$request->forgot_password) {
                if ($otp_on == 'email') {
    ...
    public function changeMobile(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data = new User();
            $user_exist = $data->where(array('mobile' => $mobile_number))->first();
            if (!empty($user_exist)) {
    ...
    public function profilePhotoUpdate(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->file('profile_picture')) {
                ...
                echo $extension = $request->file('profile_picture')->extension();
                $filepath = Storage::disk('public')->putFileAs('profile_image', $request->file('profile_picture'), time() . '.' . $extension);*/
