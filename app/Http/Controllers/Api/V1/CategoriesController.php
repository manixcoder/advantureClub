...
class CategoriesController extends MyController {
    ...
    public function get(Request $request, $id = null) {
        ...
        if ($id) {
            $data = Service_categorie::select(['id', 'category', 'image', 'status'])->where(['id' => $request->id, 'deleted_at' => null])->first();
        } else {
            $data = Service_categorie::select(['id', 'category', DB::raw("CONCAT('" . $url . "/',image) as image"), 'status'])->get();
        }
        if (!$data->isEmpty()) {
